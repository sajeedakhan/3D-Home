...
GLfloat diffuse6[]={1,1,.5,1};
GLfloat shininess6[]={50};
matprop(ambient6,diffuse6,specular6,shininess6);*/
glPushMatrix();
...
GLfloat ambient5[]={1,0,1,1};
GLfloat specular5[]={1,1,1,1};
GLfloat diffuse5[]={.5,1,.5,1};
GLfloat shininess5[]={50};
matprop(ambient5,diffuse5,specular5,shininess5);
glPushMatrix();
glPushMatrix();
...
GLfloat specular2[]={1,1,0,1};
GLfloat diffuse2[]={1,1,.5,1};
GLfloat shininess1[]={50};
matprop(ambient2,diffuse2,specular2,shininess1);
//left wall
GLfloat ambient6[]={1,0,0,1};
GLfloat specular6[]={1,0,0,0};
GLfloat diffuse6[]={1,1,.7,1};
GLfloat shininess6[]={50};
matprop(ambient6,diffuse6,specular6,shininess6);
glPushMatrix();
glTranslated(0,0,-.02);
...
glPopMatrix();
//right wall
GLfloat ambient7[]={1,1,0,1};
GLfloat specular7[]={0,1,0,1};
GLfloat diffuse7[]={1,1,.7,1};
...
{
int i;
static float gateTranslation = 0.0f; 
...
if (mgo == 0)
    {
        // Slowly translate the gate's position over time
        if (gateTranslation < 1.5f)
        {
            // Introduce a delay or loop to control the incrementation
            static float translationStep = 0.01f;
            static int delay = 10;  // Adjust the delay value for the desired speed
            ...
            {
                gateTranslation += translationStep;
                delay = 10;  // Reset the delay value
            }
        }
    }
    else
    {
        // Slowly translate the gate's position back to the closed position
        if (gateTranslation > 0.0f)
        {
            // Introduce a delay or loop to control the decrementation
            static float translationStep = 0.01f;
            static int delay = 10;  // Adjust the delay value for the desired speed
...
glTranslated(-1.3-gateTranslation, 0, 6);
glTranslated(1.5,0,0);  
//glTranslated(1.5,0,0);
...
void sgate(void )
{
static float subGateRotation = 0.0f;  // Variable to control sub gate rotation
    static float subGateTranslation = 0.0f;  // Variable to control sub gate translation
    static int delay = 10;  // Delay between each step
    int i;
    ...
    // To open the sub gate gradually
    if (sgo == 50 && subGateRotation < 90.0f)
    {
        // Slowly rotate the sub gate over time
        if (delay <= 0)
        {
            subGateRotation += 1.0f;  // Adjust the rotation step for the desired speed
    ...
    // To close the sub gate gradually
    else if (sgo == 0 && subGateRotation > 0.0f)
    {
        // Slowly rotate the sub gate in the opposite direction over time
        if (delay <= 0)
        {
            subGateRotation -= 1.0f;  // Adjust the rotation step for the desired speed
            delay = 10;  // Reset the delay
    ...
    glTranslated(5.75 + 0.25 + subGateTranslation, 0.05, 6);
    glRotated(subGateRotation, 0, 1, 0);  // Apply the sub gate rotation
    glTranslated(-5.75 - 0.25 - subGateTranslation, -0.05, -6);
    glTranslated(5.75, 0.05, 6);
    
   /* glTranslated(5.75 - 0.25 + subGateTranslation, 0.05, 6);
    glRotated(subGateRotationStep, 0, 1, 0);  // Apply the sub gate rotation
    glTranslated(-5.75 + 0.25 - subGateTranslation, -0.05, -6);
    glTranslated(5.75, 0.05, 6);*/
    
//glTranslated(5.75 - 0.25, 0.05, 6);
//glRotated(subGateRotationStep, 0, 1, 0);  // Apply the sub gate rotation
//glTranslated(-5.75 + 0.25, -0.05, -6);
//glRotated(subGateRotationStep, 0, 1, 0);
//glTranslated(5.75, 0.05, 6);
...
glPopMatrix();
//right wall
GLfloat ambient7[]={1,1,0,0};
GLfloat specular7[]={1,0,0,1};
GLfloat diffuse7[]={1,1,.7,1};
GLfloat shininess7[]={50};
matprop(ambient7,diffuse7,specular7,shininess7);
glPushMatrix();
